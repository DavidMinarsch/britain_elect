...
params_m2_a = pd.DataFrame(o_dict)
o_dict = OrderedDict({'Sex ' + str(i+1): params_m2['b'][:, i] for i in range(0, params_m2['b'].shape[1])})
params_m2_b = pd.DataFrame(o_dict)
o_dict = OrderedDict({'Age ' + str(i+1): params_m2['c'][:, i] for i in range(0, params_m2['c'].shape[1])})
params_m2_c = pd.DataFrame(o_dict)
o_dict = OrderedDict({'Ethnicity ' + str(i+1): params_m2['d'][:, i] for i in range(0, params_m2['d'].shape[1])})
params_m2_d = pd.DataFrame(o_dict)
...
pc.plot_coefficients(params=params_m2_demo,
                     ticks_list=list(params_m2_demo.columns.values),
                     title='Coefficients',
                     f_name='plots/DemoCoefficients_ConfidenceIntervals_m2.png')

# Plot coefficients with confidence intervals:
pc.plot_coefficients(params=params_m2_region,
                     ticks_list=list(params_m2_region.columns.values),
                     title='Region Intercepts',
                     f_name='plots/StateIntercepts_ConfidenceIntervals_m2.png')
...
for l in range(0, L):
    y_pred[:, l] = sp.special.expit(alpha_m1.ix[:, 0]
                                    + alpha_m1.ix[:, 1] * ge_10_region_share['main'][census_11_numeric.region[l]]
                                    + a_m1.ix[:, census_11_numeric.region[l] - 1]
                                    + b_m1.ix[:, census_11_numeric.sex[l] - 1]
                                    + c_m1.ix[:, census_11_numeric.age[l] - 1]
                                    + d_m1.ix[:, census_11_numeric.ethnicity[l] - 1])
    y_pred_cond[:, l] = sp.special.expit(alpha_m2.ix[:, 0]
                                         + alpha_m2.ix[:, 1] * ge_10_region_share['main'][census_11_numeric.region[l]]
                                         + a_m2.ix[:, census_11_numeric.region[l] - 1]
                                         + b_m2.ix[:, census_11_numeric.sex[l] - 1]
                                         + c_m2.ix[:, census_11_numeric.age[l] - 1]
...
for j in range(1, n_region + 1):
    ...
    y_pred_region_con[:, j-1] = np.divide((np.dot(y_con[:, sel],
                                           (census_11_numeric[census_11_numeric.region == j]).N)),
                                          sum((census_11_numeric[census_11_numeric.region == j]).N))
y_pred_region_con = pd.DataFrame(y_pred_region_con)

y_pred_region_lab = np.full((int((n_iter / 2) * n_chains), n_region), np.nan)
for j in range(1, n_region + 1):
    sel = [s for s in range(L) if census_11_numeric.region[s] == j]
    y_pred_region_lab[:, j-1] = np.divide((np.dot(y_lab[:, sel],
                                           (census_11_numeric[census_11_numeric.region == j]).N)),
                                          sum((census_11_numeric[census_11_numeric.region == j]).N))
...
for j in range(1, n_region + 1):
    sel = [s for s in range(L) if census_11_numeric.region[s] == j]
    y_pred_region_other[:, j-1] = np.divide((np.dot(y_other[:, sel],
                                             (census_11_numeric[census_11_numeric.region == j]).N)),
                                            sum((census_11_numeric[census_11_numeric.region == j]).N))
...
plt.hlines(range(y_pred_region_con.shape[1]),
           y_pred_region_con.quantile(0.025),
           y_pred_region_con.quantile(0.975),
           'b')
plt.hlines(range(y_pred_region_con.shape[1]),
           y_pred_region_con.quantile(0.25),
           y_pred_region_con.quantile(0.75),
           'b',
           linewidth=3)
plt.hlines(range(y_pred_region_other.shape[1]),
           y_pred_region_other.quantile(0.025),
           y_pred_region_other.quantile(0.975),
           'y')
plt.hlines(range(y_pred_region_other.shape[1]),
           y_pred_region_other.quantile(0.25),
           y_pred_region_other.quantile(0.75),
           'y',
           linewidth=3)
plt.hlines(range(y_pred_region_lab.shape[1]),
           y_pred_region_lab.quantile(0.025),
           y_pred_region_lab.quantile(0.975),
           'r')
plt.hlines(range(y_pred_region_lab.shape[1]),
           y_pred_region_lab.quantile(0.25),
           y_pred_region_lab.quantile(0.75),
           'r',
           linewidth=3)
