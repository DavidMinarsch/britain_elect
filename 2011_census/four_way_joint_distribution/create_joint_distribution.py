    ...
    for r in range(0, n_region):
      for a in range(0, n_age):
        ...
        if not isinstance( val, np.int64 ):
          val = int(val.replace(',', ''))
        data = pd.DataFrame({'sex': [s], 'ethnicity': [e], 'region': [r], 'age': [a], 'N': [val]})
        census_11 = census_11.append(data)
        region = selection.columns[r + 1]
        age = selection.iloc[a, 0]
        data = pd.DataFrame({'sex': [sex], 'ethnicity': [ethnicity], 'region': [region], 'age': [age], 'N': [val]})
        census_11_str = census_11_str.append(data)

replace_map = {"East": "East of England", "Yorkshire and The Humber": "Yorkshire and the Humber", 'East Midlands': 'East Midlands',
               'London': 'London', 'North East': 'North East', 'North West': 'North West', 'South East': 'South East', 'South West': 'South West',
...
census_11.to_csv("/Users/davidminarsch/Desktop/PythonMLM/Election_Ex_Britain/2011_census/four_way_joint_distribution/four_way_joint_distribution.csv", index=False)
census_11_str.to_csv("/Users/davidminarsch/Desktop/PythonMLM/Election_Ex_Britain/2011_census/four_way_joint_distribution/four_way_joint_distribution_str.csv", index=False)
